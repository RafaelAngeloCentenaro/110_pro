...
while True:
	...
	if status:
		...
		cam=cv2.resize(frame,(224,224))
		# Expanda a dimensão do array junto com o eixo 0
		test_cam=np.array(cam,dtype=np.float32)
		test_cam=np.expand_dims(test_cam,axis=0)
		# Normalize para facilitar o processamento
		normalized_cam=test_cam/255.0
		# Obtenha previsões do modelo
		prediction=model.predict(normalized_cam)
